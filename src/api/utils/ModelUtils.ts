...
const getPopulateArray = (query: [], allowedFields: string[]) => {
  let queryArray: any = query;
  if (!queryArray) {
    return [];
  }

  // convet query to array when we have just one populate field
...
export function listData(context: any, query: any, allowedFields: string[]) {
  ...
  if (mongoQueryObj.corporate == "null"){
    mongoQueryObj.corporate= null
  }
