...
// Config
const { JWT_EXPIRATION_MINUTES, slackEnabled, emailEnabled, isAdmin } = require('config/vars');
...
exports.register = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const user = await new User({ ...req.body, ...type }).save();
    const userTransformed = user.transform();
    const token = generateTokenResponse(user, user.token());
    const citizen = await new User({ user: user, corporate: req.params.id }).save(); // this was citizen !!!!!!!!!!!
    res.status(CREATED);
...
exports.login = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { user, accessToken } = await User.findAndGenerateToken(req.body, USER);
    const token = generateTokenResponse(user, accessToken);

    if (isAdmin(user) && slackEnabled) {
